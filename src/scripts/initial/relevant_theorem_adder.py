...
def parse_midterms_content(content):
    ...
    for section_title, section_content in sections:
        ...
        questions = re.findall(
            r"\\subsection\{(.+?)\}(.+?)(?=\\subsection|$)", section_content, re.DOTALL
        )
        parsed_sections[section_title] = []
        for question_title, question_content in questions:
            relevant_theorems = re.findall(
                r"\\textbf{Relevant Theorems: }(.+?)\\", question_content
            )
            if relevant_theorems:
                relevant_theorems = [
                    theorem.strip() for theorem in relevant_theorems[0].split(",")
                ]
            else:
                relevant_theorems = []
            parsed_sections[section_title].append(
                (question_title.strip(), relevant_theorems)
            )
...
def update_index_file(index_file_path, theorems):
    ...
    end_of_front_matter_index = next(
        (i for i, line in enumerate(lines) if line.strip() == "---" and i > 0), None
    )

    # Find the tags line
    tag_line_index = next(
        (i for i, line in enumerate(lines) if line.startswith("tags:")), None
    )

    if tag_line_index is not None and end_of_front_matter_index is not None:
        for theorem in theorems:
            tag_line_index += 1
            while tag_line_index < end_of_front_matter_index and lines[
                tag_line_index
            ].strip().startswith("-"):
                tag_line_index += 1
...
if __name__ == "__main__":
    ...
    )  # This should be the 'comp2804' directory
